...
def iou_mapping(box_yolo,gt_boxes):
    ...
    if max_iou > 0.5:
        gt = gt_boxes[ind]
        #dict_csv = {'gt_x':gt[0],'gt_y':gt[1],'gt_xmax':gt[2],'gt_ymax':gt[3],'iou':max_iou}
...
def detector(image, num, gt_boxes):
    ...
    for i in pindex:
        ...
        if person_class_ids[i]==0:
            ...
            w = person_boxes[it][2]
            h = person_boxes[it][3]
            #persons_in_image.append({'x':x,'y':y,'xmax':w+x,'ymax':h+y,'conf':str(person_confidences[it])[0:4]})
            #dict_csv = {'Sno':glob_count,'Image_num':num,'x':x,'y':y,'xmax':w+x,'ymax':h+y,'conf':str(person_confidences[it])[0:4]}
